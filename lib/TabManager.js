var TabManager = React.createFactory(React.createClass({
  ...
  update: function () {
    chrome.windows.getAll({populate: true}, function (windows) {
      ...
      for (var id in selection) {
        if (selection.hasOwnProperty(id) && !this.state.tabsById[id])
  ...
  search: function (e) {
    var calledByUser = typeof e === "string";
    var searchStr = calledByUser ? e : (e.target.value || "");
    var searchLen = searchStr.length;
    var lastSearchStr = localStorage["last-search"] || "";
    var lastSearchLen = lastSearchStr.length;
    var hiddenCnt = this.state.hiddenCount || 0;
    ...
    if (!searchLen) { // reset
      ...
      var idList = null;
      if (!calledByUser && lastSearchLen > 0) {
        if (lastSearchLen > searchLen && lastSearchStr.indexOf(searchStr) !== -1) {
          idList = this.state.hiddenTabs;
        } else if (lastSearchLen < searchLen && searchStr.indexOf(lastSearchStr) !== -1) {
          idList = this.state.selection;
        } else if (lastSearchLen === searchLen && searchStr === lastSearchStr) {
          return; // do nothing
        }
      }
      if (idList === null) {
      ...
      for (var id in idList) {
        ...
        if ((tab.title + " " + tab.url).toLowerCase().indexOf(searchStr.toLowerCase()) >= 0) {
          hiddenCnt -= (this.state.hiddenTabs[id] || 0);
          this.state.selection[id] = true;
          delete this.state.hiddenTabs[id];
        } else {
          hiddenCnt += 1 - (this.state.hiddenTabs[id] || 0);
          this.state.hiddenTabs[id] = true;
          delete this.state.selection[id];
        }
      }
    }
    localStorage["last-search"] = searchStr;
    window.setActiveTab(null);
    window.keyboardCursorPos = -1;
    this.state.hiddenCount = hiddenCnt;
    this.forceUpdate(null);
  },
